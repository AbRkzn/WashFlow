...
- **Photos** — before/after binaries are queued in a separate low-priority table and uploaded to Supabase Storage *outside* the critical sync cycle, so a large photo never starves row sync. Rows referenced by photo are hydrated back to other devices on demand.
